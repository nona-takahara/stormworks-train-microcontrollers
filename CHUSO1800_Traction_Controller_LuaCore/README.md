...
当初案では SAP/ECB切替・過速度しきい値・力行電流リミットをすべてLuaの
ソース定数としてハードコードしていたが、Stormworksの
...
（M-type（1800/1900）を表す `property.getBool("M type")` は、それを使う
パンタグラフ関連ロジックがゲート側に残ったため、本モジュールでは読まない。）
...
2. 置き換え済みとなるゲート網を削除する：phase1/phase2/regenのSRラッチと
   ...
   `bc_target_smooth`。**パンタグラフラッチ4個は削除対象ではない**
   （ゲート側に残す設計に変更したため）。
...
4. 残すゲート（パンタグラフラッチ・架線電圧セレクタ・Momelink整形・
   ...
   その配線は元のままでよい。
5. `sap_ecb_toggle`／`overspeed_threshold`／`power_limit_current` の各
   `PROPERTY_*` ノード自体は削除せずそのまま残す（本モジュールの
   `property.getBool`/`property.getNumber` 呼び出しがこれらと同名の
   プロパティを読みに行くため、ノードを消すとプロパティの定義自体が
   なくなってしまう）。`mtype_toggle`／`is_1800_type`／`notch_fb_ge1` など、
   パンタグラフ用も含め今回Lua側へ吸収されなかった／されたゲートの扱いは
   上記2・4の通り。
