...
これら小関数（および`M.physics_tick`/`M.decode_state`/`M.encode_state`/
`M.encode_stateless_in`/`M.decode_stateless_out`）は、名前付きテーブルでは
...
`deploy/main.lua`は`require("state_sync")`／`local core =
require("chuso1800_core")`でこの2ファイルを読み込む（`require`と`dofile`の
使い分け・storm-lua-minifyの`-m`モードでの挙動差は`main.lua`冒頭コメント
参照 ─ `require`の方が出力サイズ・正しさの両面で有利だった）。実機投入用の
最終フラット化・minifyは`deploy/build.js`（Node.js、Windowsでも同じ手順で
動く）が`storm-lua-minify`パッケージを呼び出して行い、
`deploy/chuso1800_deploy.lua`を生成する。これがStormworksの単一LUAノードに
そのまま貼り付けられる完成品で、生成物自体もリポジトリにコミットしてある
（3つのソースいずれかを変更したら`node build.js`で再生成すること）。

**8192文字制限との戦い**：Stormworksの1つのLUAノードには最大8192文字までしか
書けない。`src/chuso1800_core.lua`は元々このminify後だけで12,386文字あり、
単体でも制限を超えていた。storm-lua-minifyは識別子（ローカル変数・関数名）は
短縮できるが、テーブルのキー文字列（`t.field_name`のような形での参照）は
短縮できないため、名前付きテーブルを多用するコードは圧縮が効きにくい。
そこで以下を行い、最終的な`deploy/chuso1800_deploy.lua`を7,873文字（8192未満）
まで縮小した（経緯・実測値の推移は `DESIGN_LOG.md` #13）：
1. ビットレイアウトの汎用テーブル機構（`pack_bits`/`unpack_bits`＋
   `*_LAYOUT`テーブル）を廃止し、ビット位置を直接指定する
   `to_u32`/`get_bits`/`put_bits`/`get_bit`/`put_bit`に置き換えた。
2. `calculateTick`内の全ヘルパー関数・`M.physics_tick`・
   `M.decode_state`/`M.encode_state`/`M.encode_stateless_in`/
   `M.decode_stateless_out`を、名前付きテーブルではなく位置引数・多値返却で
