...
残った `notch_pos`／`controller_stop`／`regen_flag` は引き続き
`INPUT_BITS_LAYOUT`（5bit）にビットパックしている ─ 予備スロットに余裕が
あるため、この3項目だけを無理に生doubleへ展開する必要はないと判断した。
詳細は `SIGNAL_MAP.md` の「ステートレス入力スロットのレイアウト」を参照。
...
- ステート：17bit（パック済みラッチ・周期カウンタ）＋19bit（パック済み
  ...
  8本中7本を使用、1本予備。
- ステートレス入力：8本中6本を使用（`speed`/`catenary_voltage_sw`/
  `brake_pressure_sw`/`sap_pressure_sw`/`direction`の5生double＋
  `notch_pos`/`controller_stop`/`regen_flag`のパック済み1スロット）、2本予備。
