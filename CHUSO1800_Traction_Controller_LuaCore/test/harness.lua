...
-- state/physics functions (see DESIGN_LOG.md #13: the core module itself
-- takes/returns plain positional values so storm-lua-minify doesn't carry
-- un-shrinkable table-key strings into the deploy build; scenario files
-- are free to use named tables since they're never part of that build).
--------------------------------------------------------------------------
...
function harness.encode_state(core, f)
    return core.encode_state(
...
function harness.decode_state(core, state_in)
    local position_counter, phase1_latch, phase2_latch, regen_latch,
        ...
        OLD_I, OLD_IF_A, OLD_PHI, regen_bc_smooth, bc_target_smooth = core.decode_state(state_in)
    return {
...
function harness.encode_stateless_in(core, f)
    return core.encode_stateless_in(f.speed, f.catenary_voltage_sw, f.brake_pressure_sw,
        f.sap_pressure_sw, f.direction, f.notch_pos, f.controller_stop, f.regen_flag)
end

function harness.decode_stateless_out(core, stateless_out)
    local motor_current, W, bc_target_smooth, bcT, cam_pulse, phase1_latch, phase2_latch, regen_latch,
        notch_ge1, low_bc_with_regen_flag, field_current_excess_cond, power_cut = core.decode_stateless_out(stateless_out)
    return {
...
function harness.physics_tick(core, p)
    local motor_current, back_emf, accel, W, iF_a, bcT, OLD_I, OLD_IF_A, OLD_PHI = core.physics_tick(
