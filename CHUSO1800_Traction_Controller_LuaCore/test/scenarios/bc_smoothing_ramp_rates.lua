...
return function(h)
    ...
    for tick = 1, 5 do
        local before = h.decode_state(core, state)
        ...
        state = new_state
        local after = h.decode_state(core, state)
    ...
    local rise_inputs = h.encode_stateless_in(core, { direction = 1, brake_pressure_sw = 5 })
    local prev = 0
    for tick = 1, 5 do
        local _, ns = core.calculateTick(rise_inputs, rise_state)
        rise_state = ns
        local st = h.decode_state(core, rise_state)
    ...
    local fall_inputs = h.encode_stateless_in(core, {
    ...
    for tick = 1, 5 do
        ...
        fall_state = ns
        local st = h.decode_state(core, fall_state)
