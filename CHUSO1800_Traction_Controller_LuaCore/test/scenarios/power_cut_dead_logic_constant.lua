...
return function(h)
    local extreme_states = {
        zero_state(),
        h.encode_state({ position_counter = 13, phase1_latch = true, OLD_I = 100000, OLD_IF_A = 500, OLD_PHI = 5 }),
        h.encode_state({ position_counter = 14, phase2_latch = true, regen_latch = true }),
        h.encode_state({ regen_delay_level = 600, regen_bc_smooth = -50, bc_target_smooth = -999 }),
    }

    local extreme_inputs = {
        h.encode_stateless_in({}),
        h.encode_stateless_in({ speed = 1000, catenary_voltage_sw = 999999, notch_pos = 7, direction = 1, brake_pressure_sw = 5 }),
        h.encode_stateless_in({ controller_stop = true, brake_pressure_sw = 0 }),
        h.encode_stateless_in({ speed = -1000, direction = -1, notch_pos = 7, brake_pressure_sw = 5 }),
    }
    ...
    for si, state in ipairs(extreme_states) do
        for ii, stateless_in in ipairs(extreme_inputs) do
            local s = state
            for tick = 1, 20 do
                local stateless_out, ns = core_tick(stateless_in, s)
                s = ns
