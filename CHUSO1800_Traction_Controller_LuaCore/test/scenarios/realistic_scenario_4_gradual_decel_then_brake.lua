-- User-specified realistic driving scenario 4 (DESIGN_LOG.md #27/#28/#29):
...
-- §7.5, the field-current-excess pulse only demotes Parallel->Series (and
-- regen only ever engages) while "DB automatic" is ON -- with it OFF the
-- vehicle disconnects from the catenary entirely instead.
...
return function(h)
    ...
    sim:phase("notch off, external gradual decel to 35km/h over 20s", {
        notch = 0, seconds = 20, db_auto = true,
