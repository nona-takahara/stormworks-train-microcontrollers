...
return function(h)
    ...
    h.assert_eq(core.decode_state(ns_full).regen_delay_level, 600, "charge clamps at 600")
    ...
    h.assert_eq(core.decode_state(ns_empty).regen_delay_level, 0, "discharge floors at 0")
    ...
    h.assert_near(core.decode_state(ns_charged).regen_bc_smooth, -0.5 + 0.02, 1e-9,
        "level 600 is charged (regen_bc_smooth ramps toward 0)")

    local not_charged_state = core.encode_state({
    ...
    h.assert_near(core.decode_state(ns_not_charged).regen_bc_smooth, -0.5 - 0.1, 1e-9,
