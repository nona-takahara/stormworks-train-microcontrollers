...
return function(h)
    ...
    local charge_state = h.encode_state(core, {
        position_counter = 5, phase1_latch = true, phase1_cap_counter = 6,
        OLD_IF_A = 300, OLD_I = 1000,
    })
    local charge_inputs = h.encode_stateless_in(core, {
    ...
    for tick = 1, 10 do
        ...
        charge_state = ns
        local st = h.decode_state(core, charge_state)
    ...
    -- short of 0.5).
    local near_full = h.encode_state(core, {
        position_counter = 5, phase1_latch = true, phase1_cap_counter = 6,
        OLD_IF_A = 300, OLD_I = 1000, regen_delay_level = 580,
    })
    local _, ns_full = core.calculateTick(charge_inputs, near_full)
    h.assert_eq(h.decode_state(core, ns_full).regen_delay_level, 600, "charge clamps at 600")
    ...
    local discharge_state = h.encode_state(core, {
    ...
    for tick = 1, 10 do
        ...
        discharge_state = ns
        local st = h.decode_state(core, discharge_state)
    ...
    local near_empty = h.encode_state(core, {
        position_counter = 5, phase1_latch = false, regen_delay_level = 0,
    })
    local _, ns_empty = core.calculateTick(idle_inputs, near_empty)
    h.assert_eq(h.decode_state(core, ns_empty).regen_delay_level, 0, "discharge floors at 0")
    ...
    local drift_inputs = h.encode_stateless_in(core, {
        speed = 5, catenary_voltage_sw = 1500, notch_pos = 0, direction = 1, brake_pressure_sw = 5,
        regen_flag = true, sap_pressure_sw = 5.5,
    })
    local charged_state = h.encode_state(core, {
        position_counter = 5, phase1_latch = false, regen_delay_level = 600, regen_bc_smooth = -0.5,
    })
    local _, ns_charged = core.calculateTick(drift_inputs, charged_state)
    h.assert_near(h.decode_state(core, ns_charged).regen_bc_smooth, -0.5 + 0.02, 1e-9,
        "level 600 is charged (regen_bc_smooth ramps toward 0)")

    local not_charged_state = h.encode_state(core, {
        position_counter = 5, phase1_latch = false, regen_delay_level = 599, regen_bc_smooth = -0.5,
    })
    local _, ns_not_charged = core.calculateTick(drift_inputs, not_charged_state)
    h.assert_near(h.decode_state(core, ns_not_charged).regen_bc_smooth, -0.5 - 0.1, 1e-9,
