-- Tests chuso1800_core's own inlined bit-field helpers (exposed as
-- core.to_u32/core.get_bits/core.put_bits for this purpose only -- see the
-- module header comment on why these are inlined rather than a separate
-- `require`d file: it must stay standalone-testable with no dependency on
...
-- arguments baked directly into M.decode_state/M.encode_state/
-- M.decode_stateless_out -- see DESIGN_LOG.md #13 for why (Stormworks'
...
return function(h)
    ...
    h.assert_eq(core.to_u32(12345), 12345, "to_u32 passes through a plain integer")
    h.assert_eq(core.to_u32(12345.7), 12345, "to_u32 floors a fractional double")
    h.assert_eq(core.to_u32(0), 0, "to_u32(0)")
    h.assert_eq(core.to_u32(2 ^ 32 + 5), 5, "to_u32 wraps past 2^32")
    ...
    h.assert_eq(core.get_bits(core.put_bits(0, 0, 3), 0, 3), 0, "put/get round trip: min value")
    h.assert_eq(core.get_bits(core.put_bits(7, 0, 3), 0, 3), 7, "put/get round trip: max 3-bit value")
    h.assert_eq(core.get_bits(core.put_bits(1023, 5, 10), 5, 10), 1023, "put/get round trip: max 10-bit value at a nonzero shift")

    local combined = core.put_bits(5, 0, 3) | core.put_bits(1, 3, 1) | core.put_bits(600, 4, 10) | core.put_bits(1, 14, 1)
    h.assert_eq(core.get_bits(combined, 0, 3), 5, "combined field a unaffected by neighbors")
    h.assert_eq(core.get_bits(combined, 3, 1), 1, "combined field b unaffected by neighbors")
    h.assert_eq(core.get_bits(combined, 4, 10), 600, "combined field c unaffected by neighbors")
    h.assert_eq(core.get_bits(combined, 14, 1), 1, "combined field d unaffected by neighbors")
    ...
    h.assert_eq(core.get_bits(core.put_bits(99, 0, 3), 0, 3), 99 & 7, "put_bits wraps (not clamps) an over-width value")
    ...
    local decoded = h.decode_state(core, state)
    ...
    local all_status_bits = core.put_bits(1, 0, 1) | core.put_bits(1, 1, 1) | core.put_bits(1, 2, 1)
        | core.put_bits(1, 3, 1) | core.put_bits(1, 4, 1) | core.put_bits(1, 5, 1)
        | core.put_bits(1, 6, 1) | core.put_bits(1, 7, 1)
    local status = h.decode_stateless_out(core, { 0, 0, 0, 0, all_status_bits, 0, 0, 0 })
    ...
    local no_status_bits = h.decode_stateless_out(core, { 0, 0, 0, 0, 0, 0, 0, 0 })
