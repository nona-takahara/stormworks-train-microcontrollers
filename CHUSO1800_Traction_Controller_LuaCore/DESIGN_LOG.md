...
## #22 実機検証に基づく`state_sync`フィードバック配線の確定

- **背景**：レビュー時に、`core_out` ch1-16を`core_input` ch17-32へ
  明示的にずらして戻す案を検討した。しかし実機検証により、旧来の
  `COMPOSITE_WRITE_NUMBER core_write(count=16, inc=core_out)`による
  ch17-32パススルー配線が正しいことが確認された。
- **理由**：`lib/state_sync.lua`単体の入出力表だけを見ると、ch17-32へ
  「確定済みの2tick遅れ値」を明示的に入れる必要があるように見える。
  しかし実際のStormworksマイコンでは、`core_out -> COMPOSITE_WRITE_NUMBER -> LUA`
  の自己ループ経路そのものに、LUAノード入出力側とゲート経路側の遅延が乗る。
  そのため`inc=core_out`でch17-32をそのまま戻す形が、結果として
  `state_sync.lua`の期待するジャスト2tick遅延になる。
- **保守上の注意**：`core_out` ch1-16を読み出して`core_input` ch17-32へ
  書き込む「チャンネルずらし」修正は、静的なチャンネル表だけを見ると
  妥当に見えるが、実機の遅延モデルでは誤り。再導入しないこと。
  このブリッジでは、ch1-16だけを現在入力/1tick遅延入力で上書きし、
  ch17-32は`inc=core_out`パススルーで保持する。
- **確認**：ユーザーによる実機検証でこの配線が正しく動作することを確認済み。
  本リポジトリ側では`typecheck-dsl`、Luaシナリオ、deploy artifact検証で
  回帰がないことを確認する。
