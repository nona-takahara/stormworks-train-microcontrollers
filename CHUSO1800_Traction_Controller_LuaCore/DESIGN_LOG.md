...
## #22 実車実装レビュー：`state_sync`フィードバック配線のチャンネルずれを修正

- **背景**：実際のゲーム内車両へ組み込むと期待通りに動かない、という報告を受け、
  Lua本体だけでなく`main.sw-net`上の`state_sync.lua`ブリッジ配線を再点検した。
  `lib/state_sync.lua`は入力ch17-24に「2tick遅れ出力fb」、ch25-32に
  「stateのフィードバック」を期待し、出力ch1-8/ch9-16にそれらを次tickへ
  戻すための確定済み値を出している。
- **問題**：旧`core_write`は`count=16`でch1-16だけを上書きし、ch17-32は
  `inc=core_out`の同一チャンネルをそのままパススルーしていた。この場合、
  `core_input` ch17-32には「前tickのcore_out ch17-32」（再同期前の当tick計算値）
  が入り、`state_sync.lua`が期待する「前tickのcore_out ch1-16」（確定済み
  2tick遅れ出力/状態）にはならない。結果として内部`s1/s2`と外部fbが毎tick
  不一致になり、再同期分岐が本来不要なtickでも走り続ける。
- **修正**：`core_out` ch1-16を`COMPOSITE_READ_NUMBER`で読み、`core_write`を
  `count=32`へ拡張して、ch17-32へ明示的に書き戻すようにした。ch1-8は現在入力、
  ch9-16は1tick遅延入力、ch17-24は確定済み出力fb、ch25-32は確定済み状態fb、
  という`state_sync.lua`の契約を`main.sw-net`上でそのまま表す。
- **保守上の注意**：ここは見た目には「incでパススルーすれば同じ」に見えるが、
  チャンネル番号を1-16から17-32へ**ずらして戻す**必要があるため同じではない。
  将来ノード数削減をするときも、`core_out ch17-32 -> core_input ch17-32`へ戻す
  形に戻してはいけない。
- **確認**：`storm-mcl typecheck-dsl project.json`で配線型を確認し、既存のLua
  シナリオテスト（`lua test/run_all.lua`、13/13）とdeploy artifact検証
  （`lua test/verify_deploy_artifact.lua`）は無回帰だった。
