...
| 3 | `sap_raw` | 生double |
| 4 | notch/EB/パンタの12bool＋ノッチ位置3bit | パック済み `INPUT_BITS_LAYOUT` |
| 5 | `"BP [atm]"`（`SAP_ECB_IS_SAP`時のみ使用） | 生double |
| 6 | `"SAP [atm]"`（`SAP_ECB_IS_SAP`時のみ使用） | 生double |
| 7 | 予備（常に0） | ─ |
| 8 | 予備（常に0） | ─ |

### `state_in[1..8]` ／ `state_out[1..8]`（tickをまたいで持ち越す状態。8本中8本使用、予備0）

| slot | 内容 | 種別 |
|---|---|---|
| 1 | カム段＋7個のSRラッチ＋2個の周期カウンタ | パック済み `STATE_LATCHES_LAYOUT` |
...
| 4 | `bcT` | 生double |
| 5 | パンタ/フェーズ状態など12bool | パック済み `STATUS_BITS_LAYOUT` |
| 6 | 予備（常に0） | ─ |
| 7 | 予備（常に0） | ─ |
| 8 | 予備（常に0） | ─ |
