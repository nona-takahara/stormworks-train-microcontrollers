...
| 6 | notch_pos(3bit)＋controller_stop・regen_flagの2bool | パック済み `INPUT_BITS_LAYOUT`（5bit） |
| 7 | 予備（常に0） | ─ |
| 8 | 予備（常に0） | ─ |
...
この結果、ステートレス入力は8本中6本を使用（`speed`・
`catenary_voltage_sw`・`brake_pressure_sw`・`sap_pressure_sw`・
`direction`の5生double＋`notch_pos`／`controller_stop`／`regen_flag`を
ビットパックした1スロット）、2本予備となった。
...
| 6 | `INPUT_BITS_LAYOUT` のパック済みビットフィールド（32bit中5bit、下記参照） | Simple IF／Controller Stop |
| 7-8 | 予備（0） | |

### `INPUT_BITS_LAYOUT`（5bit）
