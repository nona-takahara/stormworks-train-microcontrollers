...
### `state_in[1]`／`state_out[1]` — `STATE_LATCHES_LAYOUT`（32bit中22bit使用）
...
**当初案からの実装上の見直し**：`PULSE(rise)` のエッジ検出
（`position_tick_pulse`、`regen_warning_pulse`）には、専用の「前回出力」
ビットは不要だった。各ブリンカの位相ビット自体がステートであり
（`state_in`＝古い位相、フレッシュに計算した新しい位相＝今tickの決定）、
立ち上がりエッジは単に `(not old_phase) and new_phase` として、同一tick内で
追加のストレージなしに計算できる。当初案より2bit分節約できている。
...
### `state_in[3..7]`／`state_out[3..7]` — 生double
