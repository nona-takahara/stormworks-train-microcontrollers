...
`traction_blinker`/`field_current_excess_blinker` は `periodic_pulse_step` という
単一の経過tickカウンタに置き換えている：このモジュールのどこも
ブリンカの生のON/OFF出力そのものを読んでおらず、そこから駆動される
周期パルス（カム進段・界磁電流超過検知パルス）だけが意味を持つため、
`BLINKER`+`PULSE(rise)` のペアより単純な形で表現できる。詳細は下記
「実装上の見直し」を参照。

（`field_current_excess_*` は旧名 `regen_warning_*`。main.sw-netの
`brake_current_fb`(channel=6)は実際には`n409.lua`の界磁電流`iF_a`を
読んでいるだけで、この条件も「回生ブレーキの警告」ではなく「ノッチオフ後も
界磁電流が閾値を超えたまま残っている状態を検知する」ものだったため、
`CHUSO1800_Traction_Controller/main.sw-net`・`SPEC.md`と合わせて改名した。
詳細は `DESIGN_LOG.md` #10。）
...
周期パルス（カム進段・界磁電流超過検知パルス）だけが意味を持つ。そこで
