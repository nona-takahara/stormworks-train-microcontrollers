...
| N1–N4 | number | 慣性演算係数 a～d（無次元） | Traction_Controller の準備済み・未接続演算で使用 |
...
| N2 | number | BC圧 [kPa] | |
| N3 | number | モータ電流 [A] | |
| N4 | number | 架線（パンタグラフ）電流 [A] | Traction Controller Lua N29 |
| N5 | number | 架線電圧 [V] | |
| N6 | number | パンタ1の高さ | Main Circuit の内部で設定 |
| N7 | number | パンタ2の高さ | Main Circuit の内部で設定 |
| N8 | number | MR絶対圧 [atm abs] | Cab Display は入力値から1を引いてゲージ表示 |
...
以下はCHUSO2000 Traction Controllerが扱うID=1911のフレーム仕様である。

| ch | 型 | 信号名 | 備考 |
|----|-----|--------|------|
| N1 | number | 摩擦制動減速度 [m/s²] | `max(BC[atm abs]-1.45,0)/3.02`。**符号は常に正**（方向を持たない） |
| N2 | number | 力行系加速度 [m/s²] | 牽引力（力行・回生等、能動的に力を加える系）による加速度。**符号は車両の物理的前後方向に従う**（前進+・後退-） |
| N15 | number | Momelink ID | 1911 と一致するか判定 |
| N23 | number | 架線電圧 [V] | M車からT車へ転送 |
| N24 | number | 1電動機あたり電流 [A] | M車からT車へ転送 |
| N25 | number | T車へ割り当てる空気ブレーキ減速度 [m/s²] | B16有効時にT車が採用 |
| N29 | number | 架線電流 [A] | M車からT車へ転送 |
| N30 | number | ユニット両数−1 | T車がN8の定数1を中継 |
| N31 | number | 最大空気ブレーキ負担 [m/s²] | T車がN9の定数3.5を中継 |
| B16 | bool | 回生ブレーキ有効 | T車のN25採用条件 |
| B17 | bool | Luaウォッチドッグ | 毎tick反転 |
| B18 | bool | 電動機動作中 | |
| B19 | bool | 回生余剰電流あり | |
| B20 | bool | 電気ブレーキ作動中 | T車は受信値を中継 |
...
| ch | 型 | 信号名 | 備考 |
|----|-----|--------|------|
| N1 | number | 摩擦制動減速度 [m/s²] | `max(BC[atm abs]-1.45,0)/3.02`。作動比ではない |
| N2 | number | 力行系加速度 [m/s²] | Lua出力を`inc`で引き継ぐ。車両前後方向の符号を持つ |
| N15 | number | Momelink ID | 1911 |
| N23–N31 | number | VVVF・ブレーキ協調データ | 上記`Momelink Line from inner Unit`と同じ |
| B16–B20 | bool | VVVF状態 | 上記`Momelink Line from inner Unit`と同じ |
