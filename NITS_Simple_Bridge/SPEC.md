...
| F4 | §3.2 0x46/N32のフィールド幅 | 公式仕様は「前方両数」列をcolspan=4(4bit)と記載するが、実装は`dt & 31`で5bitマスクしている | 未確定（作者コメント: ミスの可能性もあるが、NITS対応不可の「16両編成」を検知するための意図的な設計の可能性もある） |
...
| F6 | n458.lua ラウンドロビン保留トリガ | 前方向フラグ(`riseB[i+8]`)の立ち上がりのみでは送信保留が起きない（後方向フラグの立ち上がりのみ判定） | 未確定・要確認（作者コメント） |
