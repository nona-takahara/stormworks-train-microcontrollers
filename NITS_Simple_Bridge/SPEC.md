...
| `to NITS` | out | composite | 自車が生成した0x41-0x47ワード（前後両方向）＋ストローブ・Simple IF由来ビットの中継 |
...
- Lua ノード（`inst LUA name (script_ref=...)`）が出力 composite のうちスクリプトが明示的に `output.set*` しないチャンネルをどう扱うか（入力からの透過があるか、常に0/falseか）について、`COMPOSITE_WRITE_*`の`inc=`パススルーは明文化されている一方、**LUAノード自体の未設定チャンネルの扱いは明文化されていない**。実機Stormworks Luaの標準仕様（透過なし、`output.set*`で明示しない限り常に0/false）に基づけば「透過なし」だが、本ツール(sw-net/storm-mcl)の解析モデルとしての挙動は未検証。該当箇所（§6, F8）は明示的に留保する。
...
- **`curB[7]`/`curB[9]`（interrupt/override）**: いずれも `input.getNumber(6)/(7)` の生値（＝Simple IF TX由来の拡張コマンド値、sw-net上 `simpleif_cmd_value_back/front`）を無変換でそのまま即時送出する、優先度の高いバイパス経路。両者の相違（何が2つに分かれているか）は本解析からは断定できず、外部トリガ元の意味の違い（§4.4で留保）。

### 2.8 送信の物理的な「新データ通知」— ストローブ
...
```
nits_tx_watchdog_not = NOT(watchdog)
nits_tx_watchdog_xor = XOR(watchdog, NOT(watchdog))   -- 常にtrue
nits_tx_watchdog_xor_not = NOT(常にtrue)               -- 常にfalse
nits_tx_strobe_cap = CAPACITOR(charge=0, discharge=0.1)(enable=常にfalse)
```

`XOR(a, NOT a)`は恒真（常にtrue）であるため`nits_tx_watchdog_xor_not`は常にfalseとなり、`nits_tx_strobe_cap`のenableは常にfalseで、CAPACITORは常時discharge側＝出力は常にfalseに収束する。**これは事実上デッドな回路であり、意図された「新データ到着時にワンショットパルスを出す」ストローブ機構としては機能していない可能性が高い**（§5参照、要検証）。
...
`front_car`/`last_car`はN32（コモンライン情報）から抽出しているが、公式仕様の0x46行では後方車両数側のフィールドが「（後方両数　伝送無し）」と明記されている。これは**現行のNITSネットワーク層では後方車両数が実際には伝送されない（常に0）ことを意味する可能性が高く**、その場合`last_car`は常に0、後述`ext_out_n5`(後側車両数)も常に0になる（§5の既知の制限として記載）。また、公式仕様の0x46行では「前方両数」列が4bit(colspan=4)と記載されているが、本スクリプトは`dt & 31`で5bitマスクしている。この差異はドキュメント側の表記か実装側かいずれに起因するか本解析だけでは断定できないため、要注意点として記録する（§5）。
...
`curB[i]`(後方向有効フラグ)または`curB[i+8]`(前方向有効フラグ)がONのチャンネルのみ処理対象とし、値変化(`chgN`)または後方向フラグの立ち上がり(`riseB[i]`)で送信保留(`sig[i].up`)を立てる。**前方向フラグの立ち上がり(`riseB[i+8]`)は保留トリガの条件に含まれていない**ため、前方向の値が変化せずフラグだけが新規に立った場合、その回では送信保留が起きない（軽微な非対称、§5）。
...
一方`ext_input_interrupt_req`(channel=12→Lua index13)と`ext_input_override_req`(channel=14→Lua index15)は、`n458.lua`のスクリプト本体を見る限り該当インデックスへの明示的な`output.setBool`呼び出しが存在しない。これらが意味のある値を読めるかどうかは、**Stormworks Lua ノードの composite 出力において、スクリプトが明示的に書き込まなかったチャンネルが入力側から透過されるか、それとも常に0/falseに初期化されるか**に依存する。`COMPOSITE_WRITE_BOOLEAN/NUMBER`ノードについては`inc=`で指定した元compositeを未書込みチャンネルについて透過することが明文化されているが、**LUAノード自体（スクリプトの`output.set*`が書かないチャンネル）についての透過規定は本リポジトリのドキュメントに存在しない**。実機Stormworksの標準Lua仕様（マイコン内Luaスクリプトの出力compositeは、そのtickで`output.set*`を呼ばなかったチャンネルは0/falseになる。入力compositeからの自動引き継ぎは無い）に従えば「透過なし」であり、その場合`ext_input_interrupt_req`/`ext_input_override_req`は常時falseの実質的なデッド配線となる。本書ではこれを**最有力の解釈**としつつ、sw-net/storm-mcl側がLUAノードを別モデルで扱っている可能性を完全には排除できないため、断定は避け留保として記録する（透過ありの場合は「NITS Extension Inputのbit12/14で割り込み・上書き送信を外部から手動トリガする」設計として意味が通る）。
...
| ID | 箇所 | 内容 | 重大度 | 状態 |
|---|---|---|---|---|
| F1 | n458.lua / n848.lua `"M%d/N%d Mode"` | sw-netで宣言されたプロパティ名は`"N%d/N%d Mode"`であり、スクリプトが参照する`"M..."`と不一致。存在しないプロパティ名の`property.getNumber`は既定で0を返すため、モード選択(3モード)が常に`Integer same`に固定され機能していない可能性が高い | 高（機能欠落の疑い） | 要ゲーム内検証 |
| F2 | n440.lua `nits_tx_strobe_cap`まわり | `XOR(watchdog, NOT(watchdog))`は恒真のため、そのNOTを取る`nits_tx_watchdog_xor_not`は常にfalseとなり、ストローブ用CAPACITORのenableが常にfalseで出力も常にfalse固定。意図された新データ通知パルスとして機能していない可能性 | 中 | 要確認（意図された設計か、実装ミスか） |
| F3 | §3.5 `ext_mode_latch`のリセット条件 | 自ID宛の異議申し立てだけでなく、編成中の無関係な他IDの0x47宣言でもラッチがリセットされてしまう。多数マイコン起動直後の拡張モード移行遅延の可能性 | 中 | 要検証（意図的な保守的動作の可能性あり） |
| F4 | §3.2 0x46/N32のフィールド幅 | 公式仕様は「前方両数」列をcolspan=4(4bit)と記載するが、実装は`dt & 31`で5bitマスクしている | 低 | ドキュメントと実装のどちらが正か要確認 |
| F5 | §3.2 後方両数の非伝送 | 公式仕様が0x46の後方両数フィールドを「伝送無し」と明記しており、事実であれば`last_car`・`NITS Extension Output`のN5(後側車両数)相当は現行ネットワーク層では常に0となる | 低〜中（実運用への影響次第） | 既知の制限として記載、要検証 |
| F6 | n458.lua ラウンドロビン保留トリガ | 前方向フラグ(`riseB[i+8]`)の立ち上がりのみでは送信保留が起きない（後方向フラグの立ち上がりのみ判定） | 低 | 軽微な非対称、実害は限定的 |
| F7 | n440.lua 0x41送信後のクリア | `riseB[1..5]`は0x41フレームの計算には使われておらず(`curB`を直接参照)、クリアのみ行われる冗長操作 | 低（デッドコード） | 実害なし |
| F8 | §6 `ext_input_interrupt_req`/`ext_input_override_req` | LUAノードの出力compositeにおける「未設定チャンネルの透過有無」という本ツール仕様に依存する。実機Stormworksの標準Lua仕様（透過なし）に従えば常時false固定の実質デッド配線と推定されるが、断定はしない | 中（設計意図に関わる） | 要ツール仕様確認（実機標準仕様なら「デッド」の可能性が高い） |
